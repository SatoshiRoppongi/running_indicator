...
print("後半の一部を除外")
print(complemented_time_text)
annotated_center_laptime_clips = [annotate(video.subclip(from_t, to_t), txt) for (
...
for lap_text in laptime_history:
    lap = [((0, lap_text['display_start']), ''),
           ((lap_text['display_start'], lap_text['display_end']), lap_text['time_text'])]
    print('rap')
    print(lap)
    annotated_laptime_history_subclip = [annotate(video.subclip(
        from_t, to_t), txt, pos=lap_text['pos'], relative=False, fontsize=10) for (from_t, to_t), txt in lap]
